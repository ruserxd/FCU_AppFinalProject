...
import io.github.jan.supabase.gotrue.providers.builtin.OAuth
...
object AuthHelper {
        ...
        return runBlocking {
            try {
                client.auth.signUpWith(Email) {
                    this.email = email
                    this.password = password
                    data = mapOf("account" to (account ?: email.split("@")[0]))
                }
                Result.success(Unit)
            } catch (e: Exception) {
                Result.failure(e)
    ...
    fun signInWithGoogle(): Result<String> {
        return runBlocking {
            try {
                val url = client.auth.signInWith(OAuth) {
                    provider = io.github.jan.supabase.gotrue.providers.OAuthProvider.GOOGLE
                }
                Result.success(url)
            } catch (e: Exception) {
                Result.failure(e)
